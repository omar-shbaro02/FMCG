...
function EvidenceView({selected,evidence}:{selected:Case|null;evidence:any}) { if(!selected)return <Empty/>; const baseline=Number(evidence?.confidence_interval_json?.derived_values?.baseline_mean); return <><PageHead eyebrow="Step 4 of 10" title="Case evidence" intro="Actuals, baseline, forecast, and uncertainty are evidence aids—not commercial conclusions."/><CaseStrip item={selected}/>{!evidence?<Empty title="Forecast evidence not generated" text="Submit the case, calculate its baseline, and run a forecast through the API workflow."/>:<div className="evidence-grid"><section className="panel chart-panel"><div className="panel-title"><div><p className="kicker">Sell-out forecast</p><h3>{evidence.series_id}</h3></div><Status tone="green">{evidence.forecast_horizon} week horizon</Status></div><MiniChart points={evidence.forecasted_values_json} baseline={Number.isFinite(baseline)?baseline:undefined}/><div className="chart-legend"><span className="actual">Point forecast</span><span className="band">90% confidence interval</span>{Number.isFinite(baseline)&&<span className="baseline">Pre-promotion baseline</span>}</div></section><section className="panel signal-list"><h3>Forecast evidence</h3>{[["Direction",evidence.forecast_direction],["Baseline comparison",evidence.baseline_comparison],["Retention",evidence.post_promo_retention_status],["Decay",evidence.decay_signal],["Uncertainty",evidence.uncertainty_level]].map(([k,v])=><div key={k}><span>{k}</span><b>{label(v)}</b></div>)}</section><section className="panel full"><h3>Data-quality notes</h3>{evidence.data_quality_notes_json.length?<ul>{evidence.data_quality_notes_json.map((n:string)=><li key={n}>{n}</li>)}</ul>:<p className="muted">No additional quality notes were produced.</p>}</section></div>}</> }
...
function MiniChart({points,baseline}:{points:ForecastPoint[];baseline?:number}) {
  const width=760,height=300,left=58,right=18,top=30,bottom=54;
  const plotWidth=width-left-right,plotHeight=height-top-bottom;
  const bounds=points.flatMap(point=>[point.lower_bound,point.upper_bound,point.point_forecast]);
  if(Number.isFinite(baseline))bounds.push(baseline!);
  const rawMin=Math.min(...bounds),rawMax=Math.max(...bounds),padding=Math.max((rawMax-rawMin)*.12,2);
  const min=Math.floor((rawMin-padding)/5)*5,max=Math.ceil((rawMax+padding)/5)*5;
  const x=(index:number)=>left+index*(plotWidth/Math.max(points.length-1,1));
  const y=(value:number)=>top+(max-value)/(max-min||1)*plotHeight;
  const ticks=Array.from({length:5},(_,index)=>max-index*((max-min)/4));
  const line=points.map((point,index)=>`${x(index)},${y(point.point_forecast)}`).join(" ");
  const upper=points.map((point,index)=>`${x(index)},${y(point.upper_bound)}`);
  const lower=[...points].reverse().map((point,index)=>`${x(points.length-1-index)},${y(point.lower_bound)}`);
  return <svg className="mini-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Six-week sell-out unit forecast with 90 percent confidence interval and pre-promotion baseline">
    ...
    <polyline points={line} className="forecast-line"/>
    {points.map((point,index)=><g key={point.week_start_date}><circle cx={x(index)} cy={y(point.point_forecast)} r="5" className="forecast-point"/><text x={x(index)} y={y(point.point_forecast)-11} textAnchor="middle" className="value-label">{point.point_forecast.toFixed(1)}</text><text x={x(index)} y={height-26} textAnchor="middle" className="axis-label">{new Date(`${point.week_start_date}T00:00:00`).toLocaleDateString(undefined,{month:"short",day:"numeric"})}</text></g>)}
  </svg>
}
