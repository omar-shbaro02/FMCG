...
type ForecastPoint = {
  week_start_date: string; point_forecast: number; lower_bound: number; upper_bound: number;
};
type HistoricalPoint = {
  week_start_date: string; sell_out_units: number; sell_in_units: number;
  stock_on_hand: number; promo_flag: boolean;
};
...
function DatasetView({token,setError,setNotice}:{token:string;setError:(s:string)=>void;setNotice:(s:string)=>void}) { const [dataset,setDataset]=useState<any>(null); const [report,setReport]=useState<any>(null); async function upload(e:FormEvent<HTMLFormElement>){e.preventDefault();const body=new FormData(e.currentTarget);try{const result=await request("/api/datasets",token,{method:"POST",body});setDataset(result);setNotice("Dataset uploaded. Run validation before using it in a case.");}catch(err){setError(err instanceof Error?err.message:"Upload failed");}} async function validate(){try{const result=await request(`/api/datasets/${dataset.id}/validate`,token,{method:"POST",body:JSON.stringify({currency:"USD",gross_margin_representation:"amount",stock_unit:"units"})});setReport(result);setNotice("Validation completed and report stored.");}catch(err){setError(err instanceof Error?err.message:"Validation failed");}} return <><PageHead eyebrow="Step 1 of 10" title="Dataset upload & validation" intro="Upload weekly FMCG evidence. Invalid rows and uncertainty remain visible; nothing is silently discarded."/><div className="two-col"><form className="panel upload-card" onSubmit={upload}><div className="upload-icon"><Upload/></div><h3>Upload commercial evidence</h3><p>CSV or XLSX · maximum 20 MB · exact weekly grain</p><a className="demo-download" href="/demo-data/fmcg-demo-portfolio.csv" download><Download/>Download full synthetic portfolio</a><label>Dataset name<input name="name" placeholder="Q2 promotion evidence" required/></label><label className="file-input"><input name="file" type="file" accept=".csv,.xlsx" required/><span><Upload/>Choose a file</span></label><button className="primary">Upload dataset</button></form><section className="panel"><h3>Validation control</h3>{!dataset?<div className="mini-empty">Upload a dataset to expose its validation controls.</div>:<><div className="dataset-ticket"><Database/><div><b>{dataset.name}</b><span>{dataset.original_filename}</span><code>{dataset.id}</code></div><Status>{label(dataset.upload_status)}</Status></div><button className="secondary wide" onClick={validate}><ClipboardCheck/>Run structured validation</button>{report&&<div className="validation-result"><Status tone={report.critical_errors.length?"red":"green"}>{label(report.overall_status)}</Status><div><span>Rows<strong>{report.row_count}</strong></span><span>Warnings<strong>{report.warnings.length}</strong></span><span>Critical<strong>{report.critical_errors.length}</strong></span></div><p>{report.forecast_eligible_series.length} forecast-eligible series</p></div>}</>}</section></div></> }

function EvidenceView({selected,evidence}:{selected:Case|null;evidence:any}) { if(!selected)return <Empty/>; const baseline=Number(evidence?.confidence_interval_json?.derived_values?.baseline_mean); const history:HistoricalPoint[]=evidence?.historical_values_json||[]; const promo=history.filter(point=>point.promo_flag); const postPromo=history.filter(point=>evidence?.promotion_end_week&&point.week_start_date>evidence.promotion_end_week); const average=(values:number[])=>values.length?values.reduce((sum,value)=>sum+value,0)/values.length:0; const ratio=Number(evidence?.confidence_interval_json?.derived_values?.sell_in_to_sell_out_ratio); return <><PageHead eyebrow="Step 4 of 10" title="Case evidence" intro="Actuals, promotion timing, baseline, forecast, and uncertainty are evidence aids—not commercial conclusions."/><CaseStrip item={selected}/>{!evidence?<Empty title="Forecast evidence not generated" text="Submit the case, calculate its baseline, and run a forecast through the API workflow."/>:<div className="evidence-grid"><section className="panel chart-panel"><div className="panel-title"><div><p className="kicker">Weekly movement timeline</p><h3>{evidence.series_id}</h3></div><Status tone="green">Actuals + {evidence.forecast_horizon} week forecast</Status></div><MiniChart points={evidence.forecasted_values_json} history={history} baseline={Number.isFinite(baseline)?baseline:undefined}/><div className="chart-legend"><span className="sellout">Actual sell-out</span><span className="sellin">Actual sell-in</span><span className="actual">Point forecast</span><span className="band">90% confidence interval</span><span className="promotion">Promotion</span>{Number.isFinite(baseline)&&<span className="baseline">Pre-promotion baseline</span>}</div><div className="evidence-metrics"><span><small>Baseline</small><b>{baseline.toFixed(1)} units/wk</b></span><span><small>Promotion peak sell-out</small><b>{Math.max(...promo.map(point=>point.sell_out_units),0).toFixed(1)} units</b></span><span><small>Post-promo sell-out</small><b>{average(postPromo.map(point=>point.sell_out_units)).toFixed(1)} units/wk</b></span><span><small>Sell-in / sell-out</small><b>{Number.isFinite(ratio)?`${ratio.toFixed(2)}×`:"N/A"}</b></span><span><small>Latest channel stock</small><b>{history.length?`${history.at(-1)!.stock_on_hand.toFixed(0)} units`:"N/A"}</b></span></div></section><section className="panel signal-list"><h3>Forecast evidence</h3>{[["Direction",evidence.forecast_direction],["Baseline comparison",evidence.baseline_comparison],["Retention",evidence.post_promo_retention_status],["Decay",evidence.decay_signal],["Uncertainty",evidence.uncertainty_level]].map(([k,v])=><div key={k}><span>{k}</span><b>{label(v)}</b></div>)}</section><section className="panel full"><h3>Data-quality notes</h3>{evidence.data_quality_notes_json.length?<ul>{evidence.data_quality_notes_json.map((n:string)=><li key={n}>{n}</li>)}</ul>:<p className="muted">No additional quality notes were produced.</p>}</section></div>}</> }
...
function MiniChart({points,history,baseline}:{points:ForecastPoint[];history:HistoricalPoint[];baseline?:number}) {
  const width=820,height=350,left=58,right=18,top=32,bottom=62;
  const plotWidth=width-left-right,plotHeight=height-top-bottom;
  const bounds=[...history.flatMap(point=>[point.sell_out_units,point.sell_in_units]),...points.flatMap(point=>[point.lower_bound,point.upper_bound,point.point_forecast])];
  if(Number.isFinite(baseline))bounds.push(baseline!);
  const rawMin=Math.min(...bounds),rawMax=Math.max(...bounds),padding=Math.max((rawMax-rawMin)*.08,5);
  const min=Math.max(0,Math.floor((rawMin-padding)/10)*10),max=Math.ceil((rawMax+padding)/10)*10;
  const combinedLength=history.length+points.length;
  const x=(index:number)=>left+index*(plotWidth/Math.max(combinedLength-1,1));
  const y=(value:number)=>top+(max-value)/(max-min||1)*plotHeight;
  const ticks=Array.from({length:5},(_,index)=>max-index*((max-min)/4));
  const sellOut=history.map((point,index)=>`${x(index)},${y(point.sell_out_units)}`).join(" ");
  const sellIn=history.map((point,index)=>`${x(index)},${y(point.sell_in_units)}`).join(" ");
  const forecastOffset=history.length;
  const forecast=points.map((point,index)=>`${x(forecastOffset+index)},${y(point.point_forecast)}`).join(" ");
  const upper=points.map((point,index)=>`${x(forecastOffset+index)},${y(point.upper_bound)}`);
  const lower=[...points].reverse().map((point,index)=>`${x(forecastOffset+points.length-1-index)},${y(point.lower_bound)}`);
  const promoIndices=history.map((point,index)=>point.promo_flag?index:-1).filter(index=>index>=0);
  const step=plotWidth/Math.max(combinedLength-1,1),promoStart=promoIndices.length?x(Math.min(...promoIndices))-step/2:0,promoEnd=promoIndices.length?x(Math.max(...promoIndices))+step/2:0;
  const dates=[...history.map(point=>point.week_start_date),...points.map(point=>point.week_start_date)];
  return <svg className="mini-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Weekly actual sell-in and sell-out, promotion period, baseline, and six-week sell-out forecast">
    <text x="12" y="18" className="axis-title">UNITS</text>
    {ticks.map(tick=><g key={tick}><line x1={left} x2={width-right} y1={y(tick)} y2={y(tick)} className="chart-grid"/><text x={left-10} y={y(tick)+4} textAnchor="end" className="axis-label">{tick.toFixed(0)}</text></g>)}
    {promoIndices.length>0&&<g><rect x={promoStart} y={top} width={promoEnd-promoStart} height={plotHeight} className="promotion-window"/><text x={(promoStart+promoEnd)/2} y={top+14} textAnchor="middle" className="promotion-label">PROMOTION</text></g>}
    <polygon points={[...upper,...lower].join(" ")} className="confidence-band"/>
    {Number.isFinite(baseline)&&<g><line x1={left} x2={width-right} y1={y(baseline!)} y2={y(baseline!)} className="baseline-line"/><text x={width-right-4} y={y(baseline!)-7} textAnchor="end" className="baseline-label">Baseline {baseline!.toFixed(1)}</text></g>}
    <polyline points={sellIn} className="sell-in-line"/><polyline points={sellOut} className="sell-out-line"/>
    {history.length>0&&points.length>0&&<line x1={x(forecastOffset)-step/2} x2={x(forecastOffset)-step/2} y1={top} y2={top+plotHeight} className="forecast-divider"/>}
    <polyline points={forecast} className="forecast-line"/>
    {points.map((point,index)=><g key={point.week_start_date}><circle cx={x(forecastOffset+index)} cy={y(point.point_forecast)} r="4" className="forecast-point"/><text x={x(forecastOffset+index)} y={y(point.point_forecast)-9} textAnchor="middle" className="value-label">{point.point_forecast.toFixed(0)}</text></g>)}
    {dates.map((value,index)=>(index%4===0||index===history.length||index===dates.length-1)?<text key={value} x={x(index)} y={height-27} textAnchor="middle" className="axis-label">{new Date(`${value}T00:00:00`).toLocaleDateString(undefined,{month:"short",day:"numeric"})}</text>:null)}
    {history.length>0&&<text x={x(Math.max(history.length-1,0))} y={height-8} textAnchor="end" className="phase-label">ACTUALS</text>}<text x={width-right} y={height-8} textAnchor="end" className="phase-label">FORECAST</text>
  </svg>
}
